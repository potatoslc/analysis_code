...
def get_flame_cut(d1,Tcut:float=305,x_cold_region:float=0):
    import yt
    import numpy as np
    import pandas as pd
    #load ds
...
def find_pathline(df, x0,y0,z0,cutoff_value,max_length,curr_dx):
    pathline = []
    pathline_dict =[]
    find_next_point_cubic(df,x0,y0,z0,cutoff_value/2,int(max_length*0.8),curr_dx,pathline,pathline_dict)
...
def find_next_point_cubic(df,x0,y0,z0,cutoff_value,max_length,curr_dx,pathline,pathline_dict):
    ...
    x_next = x0  - x_dirc/norm_dist * 0.25
    y_next = y0  - y_dirc/norm_dist * 0.25
    z_next = z0  - z_dirc/norm_dist * 0.25
    
    #x_next = x0  - x_dirc/norm_dist_sqrd * (0.000015)/curr_factor**1.5
    #y_next = y0  - y_dirc/norm_dist_sqrd * (0.000015)/curr_factor**1.5
    #z_next = z0  - z_dirc/norm_dist_sqrd * (0.000015)/curr_factor**1.5
    if z_next <=0:
        return
...
def find_prvs_point_cubic(df,x0,y0,z0,cutoff_value,max_length,curr_dx,pathline,pathline_dict):
    ...
    norm_dist_sqrd = norm_dist**2
    curr_factor = 1/norm_dist if norm_dist<1 else 1
    x_next = x0  + x_dirc/norm_dist_sqrd * (0.00005)
    y_next = y0  + y_dirc/norm_dist_sqrd * (0.00005)
    z_next = z0  + z_dirc/norm_dist_sqrd * (0.00005)
    if z_next <=0:
        return
    #print(f'The next point is x: {x_next}, y: {y_next} , z: {z_next}')
...
def flame_cline_df(df1,df_coord,slabel="",savedir = "./"):
    ...
    for i in range(df_coord.shape[0]):
        ...
        tpl2,tpd2 = find_pathline(df1,  temp_xs  ,  temp_ys ,  temp_zs  ,0.0035,400,34.1e-6  )
