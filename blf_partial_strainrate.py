...
def save_target_summary(P, T, phi, xH2, strain_ratio, Kext, target_K, actual_K, target_velocity):
    summary_file = outputdir + 'target_strain_summary'

    if not os.path.exists(summary_file):
        file = open(summary_file, 'a')
        header = 'P\tT\tphi\txH2\tstrain_ratio\tKext\ttarget_K\tactual_K\tvelocity\n'
        file.write(header)
        file.close()
    ...
    data[0, 2] = phi
    data[0, 3] = xH2
    data[0, 4] = strain_ratio
    data[0, 5] = Kext
    data[0, 6] = target_K
    data[0, 7] = actual_K
    data[0, 8] = target_velocity

    file = open(summary_file, 'ab')
    savetxt(file, data, delimiter='\t')
    file.close()
...
def save_target_strain_flames(P, T, xH2, phi, Kext, max_burn_vel):
    for target_strain_ratio in target_strain_ratios:
        ...
        best_oppFlame.save(filename_target + '.csv', basis='mole', overwrite=True)
        ...
        save_target_summary(
            ...
            phi=phi,
            xH2=xH2,
            strain_ratio=target_strain_ratio,
            Kext=Kext,
            target_K=target_K,
            actual_K=best_K,
            target_velocity=best_velocity
        )
